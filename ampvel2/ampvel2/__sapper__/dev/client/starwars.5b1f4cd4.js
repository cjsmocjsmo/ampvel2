...
/* src/routes/starwars.svelte generated by Svelte v3.30.0 */

const { console: console_1 } = globals;
const file = "src/routes/starwars.svelte";
...
function create_fragment(ctx) {
	...
	const block = {
		...
		l: function claim(nodes) {
			...
			t2 = claim_text(h1_nodes, "Star Wars");
...
function instance($$self, $$props, $$invalidate) {
	...
	validate_slots("Starwars", slots, []);
	let NewDat = [];

	onMount(async () => {
		await fetch(`http://192.168.0.42:8888/intStarWars`, { mode: "cors", method: "GET" }).then(r => r.json()).then(data => {
	...
	Object.keys($$props).forEach(key => {
		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== "$$") console_1.warn(`<Starwars> was created with unknown prop '${key}'`);
	});
...
class Starwars extends SvelteComponentDev {
	constructor(options) {
		super(options);

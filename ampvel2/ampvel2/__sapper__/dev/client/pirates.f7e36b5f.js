...
/* src/routes/pirates.svelte generated by Svelte v3.30.0 */

const { console: console_1 } = globals;
const file = "src/routes/pirates.svelte";
...
function create_fragment(ctx) {
	...
	const block = {
		...
		l: function claim(nodes) {
			...
			t2 = claim_text(h1_nodes, "Pirates");
...
function instance($$self, $$props, $$invalidate) {
	...
	validate_slots("Pirates", slots, []);
	let NewDat = [];

	onMount(async () => {
		await fetch(`http://192.168.0.42:8888/intPirates`, { mode: "cors", method: "GET" }).then(r => r.json()).then(data => {
	...
	Object.keys($$props).forEach(key => {
		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== "$$") console_1.warn(`<Pirates> was created with unknown prop '${key}'`);
	});
...
class Pirates extends SvelteComponentDev {
	constructor(options) {
		super(options);
